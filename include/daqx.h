...
DAQAPI DaqHandleT WINAPI daqOpen(PCHAR daqName);
...
/*  Utility Function Prototypes */
	...
	DAQAPI DaqError WINAPI daqGetHardwareInfo(DaqHandleT handle,
						  DaqHardwareInfo whichInfo,
						  PVOID info);

	DAQAPI DaqError WINAPI daqGetInfo(DaqHandleT handle,
					  DWORD chan,
					  DaqInfo whichInfo,
					  PVOID info);
	...
	DAQAPI DaqError WINAPI daqGetDeviceList(DaqDeviceListT * deviceList,
						DWORD * deviceCount);

	DAQAPI DaqError WINAPI daqGetDeviceProperties(PCHAR daqName,
...
/* Bank Configuration Prototypes */
	DAQAPI DaqError WINAPI daqAdcExpSetBank(DaqHandleT handle,
						DWORD chan,
						DaqAdcExpType bankType);

/* Obsolete. Use daqSetOption */
	DAQAPI DaqError WINAPI daqAdcExpSetChanOption(DaqHandleT handle,
						      DWORD chan,
						      DaqChanOptionType optionType,
						      float optionValue);

	DAQAPI DaqError WINAPI daqAdcExpSetModuleOption(DaqHandleT handle,
							DWORD chan,
							DaqChanOptionType optionType,
							float optionValue);

/*
 * Universal function for setting channel and module options in
 * WaveBooks and Daq* Devices flags defined in DaqChanOptionFlagType
 * Some Option Types have enumerated Option Values
 * (DaqChanOptionValue)
 */
	DAQAPI DaqError WINAPI daqSetOption(DaqHandleT handle,
					    DWORD chan,
					    DWORD flags,
					    DaqOptionType optionType,
					    float optionValue);
/*****************************************************************/
/*                                                               */
...
typedef enum {
   ...
   Dbk90TypeJ         = 0,
   Dbk90TypeK         = 0,
   Dbk90TypeT         = DgainX2DbkNone,
   Dbk90TypeE         = 0,
   Dbk90TypeN28       = DgainX2DbkNone,
   Dbk90TypeN14       = 0,
   Dbk90TypeS         = DgainX2DbkNone,
   Dbk90TypeR         = DgainX2DbkNone,
   Dbk90TypeB         = DgainX2DbkNone,
...
/* Replaces the daqAdcCalcTrig, daqAdcSetTrig combination */
    DAQAPI DaqError WINAPI daqSetTriggerEvent(DaqHandleT handle,
...
/* WaveBook function for setting complex triggering */
	DAQAPI DaqError WINAPI daqAdcSetTrigEnhanced(DaqHandleT handle,
						     DaqAdcTriggerSource * trigSources,
						     DaqAdcGain * gains,
						     DaqAdcRangeT * adcRanges,
						     DaqEnhTrigSensT * trigSensitivity,
						     float * level,
						     float * hysteresis,
						     PDWORD channels,
...
/* Calculate trigger level in counts  for daqAdcSetTrig */
	DAQAPI DaqError WINAPI daqAdcCalcTrig(DaqHandleT handle,
					      BOOL bipolar,
					      float gainVal,
					      float voltageLevel,
					      PWORD triggerLevel);
...
/* Recommended for Immediate or Software Trigger Only */
	DAQAPI DaqError WINAPI daqAdcSetTrig(DaqHandleT handle,
					     DaqAdcTriggerSource triggerSource,
					     BOOL rising,
					     WORD level,
					     WORD hysteresis,
					     DWORD channel);
...
/* Custom ADC Acquisition Prototypes - Set Clock Source and Scan Rate */
	DAQAPI DaqError WINAPI daqAdcSetClockSource(DaqHandleT handle,
						    DaqAdcClockSource clockSource);

	DAQAPI DaqError WINAPI daqAdcSetRate(DaqHandleT handle,
					     DaqAdcRateMode mode,
					     DaqAdcAcqState state,
					     float reqValue,
					     float * actualValue);
/* Superceded by daqAdcSetRate */
	DAQAPI DaqError WINAPI daqAdcSetFreq(DaqHandleT handle, float freq);
	DAQAPI DaqError WINAPI daqAdcGetFreq(DaqHandleT handle, float * freq);
...
/* Custom ADC Acquisition Prototypes - Direct-to-Disk */
	DAQAPI DaqError WINAPI daqAdcSetDiskFile(DaqHandleT handle,
						 PCHAR filename,
						 DaqAdcOpenMode openMode,
						 DWORD preWrite);
...
/* One-Step ADC Acquisition Prototypes */
	DAQAPI DaqError WINAPI daqAdcRd(DaqHandleT handle,
					DWORD chan,
					PWORD sample,
					DaqAdcGain gain,
					DWORD flags);

	DAQAPI DaqError WINAPI daqAdcRdScan(DaqHandleT handle,
					    DWORD startChan,
					    DWORD endChan,
					    PWORD buf,
					    DaqAdcGain gain,
	...
	DAQAPI DaqError WINAPI daqAdcRdN(DaqHandleT handle,
					 DWORD chan,
					 PWORD buf,
					 DWORD scanCount,
					 DaqAdcTriggerSource triggerSource,
	...
	DAQAPI DaqError WINAPI daqAdcRdScanN(DaqHandleT handle,
					     DWORD startChan,
					     DWORD endChan,
					     PWORD buf,
					     DWORD scanCount,
...
/* DAC Waveform Prototypes - Trigger, Update Rate and Count */
	...
	DAQAPI DaqError WINAPI daqDacWaveSetTrig(DaqHandleT handle,
						 DaqDacDeviceType deviceType,
						 DWORD chan,
						 DaqDacTriggerSource triggerSource,
						 BOOL rising);
...
/* Sets A/D min and max range for conversion routines */
	DAQAPI DaqError WINAPI daqCvtSetAdcRange(float Admin,
						 float Admax);

	DAQAPI DaqError WINAPI daqCvtLinearSetup(DWORD nscan,
						 DWORD readingsPos,
						 DWORD nReadings,
						 float signal1,
						 float voltage1,
	...
	DAQAPI DaqError WINAPI daqCvtLinearConvert(PWORD counts,
						   DWORD scans,
						   float * fValues,
						   DWORD nValues);

	DAQAPI DaqError WINAPI daqCvtLinearSetupConvert(DWORD nscan,
							DWORD readingsPos,
							DWORD nReadings,
							float signal1,
							float voltage1,
...
DAQAPI DaqError WINAPI daqReadCalFile(DaqHandleT handle,
					      PCHAR calfile);
...
DAQAPI DaqError WINAPI daqDumpDriverDebugBuffer(DaqHandleT handle,
							PCHAR buf,
							DWORD charCount);
